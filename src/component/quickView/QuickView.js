...
const QuickView = ({ item, setIsModalOpen }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="lg:w-1/2 md:w-2/5 w-full h-1/2 md:h-full bg-white flex flex-row justify-center items-center p-2">
          <img src={item.image} className=" w-auto max-h-full" alt="item img" />
        </div>
        <div className="flex flex-col justify-between md:w-3/5 lg:w-1/2 w-full h-1/2 md:h-full py-5 px-6 bg-white">
          <div>
            <span className=" font-medium text-xl block">{item.title}</span>
            <span className=" block font-medium text-gray-700 py-4">
              {item.category}
            </span>
            <span className=" sm:block hidden">{item.description}</span>
          </div>
